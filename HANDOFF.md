...
เอกสารส่งต่องานสำหรับเซสชันถัดไป — โค้ดจริงทั้งหมดอยู่ใน `index.html` ไฟล์เดียวจบ
...
- **mixer ไม่เข้า bank** (`bankSnd()` ตัด `m` ออกทั้งตอนอ่านและตอนเขียน)
  mixer เป็นของ *เครื่อง* ไม่ใช่ของ *pattern* · เดิม slot เก็บ `m` ไปด้วย พอ save ตอน fader ปิดอยู่
  การโหลด slot นั้นก็ดึง fader ลงศูนย์ทั้งเจ็ดตัว → เงียบทั้งเครื่อง และโหลด pattern อื่นตามก็ไม่กลับมา
  เพราะ mixer เป็น global · ซ้ำร้าย `saveHash()` เขียนศูนย์ลง URL ต่อ เปิดหน้าใหม่ก็ยังเงียบ
  **ตัดตอนอ่านด้วย ไม่ใช่แค่ตอนเขียน** — slot ที่ build เก่าบันทึกไว้มีศูนย์ติดอยู่แล้ว ต้องปลดชนวนโดยไม่ต้องให้ใครมา save ใหม่
  `snd` ยังเก็บ knob ไว้ตามเดิม เพราะ knob เป็นส่วนหนึ่งของ patch จริงๆ · **Copy Link ยังพา mixer ไปด้วย** แชร์เพลงคือแชร์สมดุลเสียงของมัน
